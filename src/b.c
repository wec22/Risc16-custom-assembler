...
void asmFile(FILE *, FILE *, short *);

#define ntoi(str)	strtol(str, NULL, 0)

//labels could be stored as a dictionary if it was available grr
...
main(int argc, char *argv[])
{
	...
	address = 0;
	asmFile(inFilePtr, outFilePtr, &address);
}
	...
	char **arg1Ptr, char **arg2Ptr)
{
	...
	if (statusString != NULL) {
		...
		if (firsttoken == NULL || firsttoken[0] == '#') { //checks for emptyline or comment
...
//all the verification steps need to take place at the same time before the
//actual opcodes are assembled
void asmFile(FILE *inFilePtr, FILE *outFilePtr, short *address) {
	char *inFileString, *outFileString;
	char *label, *opcode, *arg0, *arg1, *arg2;
	char *statusString;
	char lineString[MAXLINELENGTH+1];
	...
	*address = startAdr;
	...
	while(readAndParse(inFilePtr, lineString, &label, &opcode, &arg0, &arg1, &arg2) != NULL) {
		...
		} else if (!strcmp(opcode, "inv")) { //custom
			num = (NAND << OP_SHIFT) | (reg(arg0) << A_SHIFT) | (reg(arg1) << B_SHIFT) | reg(arg1);

		} else if (!strcmp(opcode, "neg")) { //custom
			num = (NAND << OP_SHIFT) | (reg(arg0) << A_SHIFT) | (reg(arg1) << B_SHIFT) | reg(arg1);
			fprintf(outFilePtr, "%04hx\n", num);
			(*address)++;
			num = (ADDI << OP_SHIFT) | (reg(arg0) << A_SHIFT) | (reg(arg1) << B_SHIFT) | imm("1");
